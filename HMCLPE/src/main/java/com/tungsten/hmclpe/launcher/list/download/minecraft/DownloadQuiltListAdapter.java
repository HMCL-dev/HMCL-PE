...
public class DownloadQuiltListAdapter extends BaseAdapter {
    ...
    public DownloadQuiltListAdapter(Context context,MainActivity activity,String mcVersion,ArrayList<QuiltLoaderVersion> versions,boolean install){
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (view == null){
            ...
            viewHolder.quiltId = view.findViewById(R.id.id);
        ...
        QuiltLoaderVersion version = versions.get(i);
        viewHolder.icon.setImageDrawable(context.getDrawable(R.drawable.ic_quilt));
        viewHolder.quiltId.setText(version.version);
        viewHolder.mcVersion.setText(mcVersion);
        viewHolder.item.setOnClickListener(v -> {
            if (install) {
                if (activity.uiManager.gameManagerUI.gameManagerUIManager.autoInstallUI.quiltVersion != null) {
                    AlertDialog.Builder builder = new AlertDialog.Builder(context);
                    builder.setTitle(context.getString(R.string.dialog_change_version_title));
                    builder.setMessage(context.getString(R.string.dialog_change_version_msg).replace("%s","Quilt").replace("%v1",activity.uiManager.gameManagerUI.gameManagerUIManager.autoInstallUI.quiltVersion).replace("%v2",version.version));
                ...
                activity.uiManager.installGameUI.quiltVersion = version;
                activity.backToLastUI();
            }
        });
        return view;
    ...
    private void update(QuiltLoaderVersion quiltLoaderVersion) {
        GameUpdateDialog dialog = new GameUpdateDialog(context,activity,activity.uiManager.gameManagerUI.gameManagerUIManager.autoInstallUI.versionName,activity.uiManager.gameManagerUI.gameManagerUIManager.autoInstallUI.gameVersion,5,quiltLoaderVersion);
        dialog.show();
    }
}
